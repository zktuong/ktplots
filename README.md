...
out <- compare_cpdb(cpdb_meta = covid_cpdb_meta, 
        			...
        			groupby = "Status_on_day_collection_summary") # the column that holds the column where you want to do the comparison. In this example, it's Severe vs Healthy
```
...
```R
# just as a dummy example, lets say the samples are matched where there two in
...
plot_compare_cpdb(out, contrast = 'Status_on_day_collection_summarySevere', groups = 'Severe')
```
