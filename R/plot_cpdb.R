...
plot_cpdb <- function(
    ...
    if (return_table) {
        ...
        g <- g + geom_point(pch = 21, na.rm = TRUE) +
            theme_bw() +
            theme(axis.text.x = element_text(angle = 45, hjust = 0, color = "#000000"), axis.text.y = element_text(color = "#000000"), axis.title.x = element_blank(), axis.title.y = element_blank()) +
