...
test_that("plot_cpdb works 1", {
    p <- plot_cpdb(cell_type1 = "B cell", cell_type2 = "CD4T cell", scdata = kidneyimmune,
...
test_that("plot_cpdb works 8", {
    p <- plot_cpdb(cell_type1 = "B cell", cell_type2 = "CD4T cell", scdata = kidneyimmune,
        celltype_key = "celltype", means = means2, pvals = pvals2, gene_family = c("chemokines",
            "th1"), keep_significant_only = FALSE)
    expect_true(is.ggplot(p))
})
...
test_that("weird characters are ok", {
    ...
    colnames(means) <- gsub("Wilms2_|TxK1_|RCC1_|RCC2_|RCC3_|Wilms3_|TxK4_|VHLRCC_|Wilms1_|Teen_|Tx_|TxK3_|TxK2_|PapRCC",
        "", colnames(means))
    colnames(pvals) <- gsub("Wilms2_|TxK1_|RCC1_|RCC2_|RCC3_|Wilms3_|TxK4_|VHLRCC_|Wilms1_|Teen_|Tx_|TxK3_|TxK2_|PapRCC",
        "", colnames(pvals))
    ...
    p <- plot_cpdb(cell_type1 = "TRC+", cell_type2 = "LTi-Like ILC3", scdata = kidneyimmune,
        celltype_key = "celltype", means = newmeans, pvals = newpvals, genes = c("LTB",
            "LTBR", "KITL", "KIT", "CCR6"), keep_significant_only = FALSE)
    expect_true(is.ggplot(p))
})

test_that("plot_cpdb2 works 1", {
    p <- plot_cpdb2(cell_type1 = "B cell", cell_type2 = "CD4T cell", scdata = kidneyimmune,
        ...
        interaction_grouping = interaction_annotation, edge_group_colors = c(Activating = "#e15759",
            Chemotaxis = "#59a14f", Inhibitory = "#4e79a7", `Intracellular trafficking` = "#9c755f",
            DC_development = "#B07aa1", Unknown = NA), node_group_colors = c(`CD4T cell` = "#86bc86",
            `B cell` = "#79706e"), keep_significant_only = TRUE, standard_scale = TRUE,
        remove_self = TRUE)
    expect_true(is.ggplot(p))
})
...
test_that("plot_cpdb3 works 1", {
    p <- plot_cpdb3(cell_type1 = "B cell", cell_type2 = "CD4T cell", scdata = kidneyimmune,
        celltype_key = "celltype", means = means2, pvals = pvals2, deconvoluted = decon2,
...
test_that("plot_cpdb4 works 1", {
    p <- plot_cpdb4(interaction = "CLEC2D-KLRB1", cell_type1 = "NK", cell_type2 = "Mast",
...
test_that("plot_cpdb4 works 2", {
    p <- plot_cpdb4(interaction = c("CLEC2D-KLRB1", "CD40-CD40LG"), cell_type1 = "NK|B",
        cell_type2 = "Mast|CD4T", scdata = kidneyimmune, celltype_key = "celltype",
        means = means2, pvals = pvals2, deconvoluted = decon2, desiredInteractions = list(c("NK cell",
            "Mast cell"), c("NK cell", "NKT cell"), c("NKT cell", "Mast cell"), c("B cell",
            "CD4T cell")), keep_significant_only = TRUE)
    expect_that(class(p), equals("recordedplot"))
})
